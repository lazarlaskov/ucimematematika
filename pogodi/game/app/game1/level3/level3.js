...
function generateForms(){
    ...
    for(var i = 0; i < 20; i ++){
        ...
        var formtype = formtypes[getRandomInt(0,1)];
        var color = colors[getRandomInt(0,4)];
        var x = getRandomInt(0, canvaswidth);
        var y = getRandomInt(0, canvasheight);
        var dx = dxdy[getRandomInt(0,1)];
...
function animationDraw(){
    ...
    ctx.font = "20px Arial";
    ctx.fillStyle = 'black';
    ctx.fillText('НИВО 3 - Погоди го ТРИАГОЛНИКОТ',50,50);
    ...
    for(var i = forms.length - 1; i >= 0; i --){
        ...
        if(evt.clientX - 40 <= forms[i].x && evt.clientX + 40 >= forms[i].x){
            if(evt.clientY + 50 >= forms[i].y && evt.clientY - 50 <= forms[i].y){
                if(forms[i].type == 'triangle'){
                    win = true;
                    if(win){
                        ctx.fillStyle = 'green';
                        ctx.fillText('ЧЕСТИТКИ! ТОЧЕН ОДГОВОР!',50,100);
                        alert('Честитки! Точен одговор! За неколку секунди ќе бидете префрлени на НИВО 4!')
                    }
                    forms.splice(i, 1);
                    break;
                }
