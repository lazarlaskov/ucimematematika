...
function animationDraw(){
    ...
    if(win){
        ...
        wait(3500);
        window.location = '../level8/level8.html';
        return;
    }
    ...
    ctx.font = "20px Arial";
    ctx.fillStyle = 'black';
    ctx.fillText('НИВО 7 - Погоди ги СИТЕ БРОЕВИ КОИ СЕ ПОМАЛИ ОД ДЕСЕТ',50,50);
...
function drawRecord(record){
    if(record.type == 'triangle'){
        ...
        ctx.font = "20px Arial";
        ctx.fillStyle = 'white';
        ctx.fillText(record.numberInside, record.x - 11, record.y + 30);
    ...
    if(record.type == 'square'){
        ...
        ctx.font = "20px Arial";
        ctx.fillStyle = 'white';
        ctx.fillText(record.numberInside, record.x + 10, record.y + 25);
    }
    if(record.type == 'circle'){
        ...
        ctx.font = "20px Arial";
        ctx.fillStyle = 'white';
        ctx.fillText(record.numberInside, record.x - 10, record.y + 7);
    }
}
            ...
            if(evt.clientY + 50 >= forms[i].y && evt.clientY - 50 <= forms[i].y){
                if(forms[i].numberInside < 10){
                    ...
                    if(win){
                        ctx.fillStyle = 'green';
                        ctx.fillText('ЧЕСТИТКИ! ТОЧЕН ОДГОВОР!',50,100);
                        alert('Честитки! Точен одговор! За неколку секунди ќе бидете префрлени на НИВО 8!')
                    }
