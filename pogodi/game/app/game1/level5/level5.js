...
var colors = ['green','red','orange','black'];
var dxdy = [-1, 1   ];
...
function animationDraw(){
    ...
    ctx.font = "20px Arial";
    ctx.fillStyle = 'black';
    ctx.fillText('НИВО 5 - Погоди го СИНИОТ КРУГ',50,50);
    ...
    for(var i = forms.length - 1; i >= 0; i --){
        ...
        if(evt.clientX - 40 <= forms[i].x && evt.clientX + 40 >= forms[i].x){
            if(evt.clientY + 50 >= forms[i].y && evt.clientY - 50 <= forms[i].y){
                if(forms[i].type == 'circle' && forms[i].color == 'blue'){
                    win = true;
                    if(win){
                        ctx.fillStyle = 'green';
                        ctx.fillText('ЧЕСТИТКИ! ТОЧЕН ОДГОВОР!',50,100);
                        alert('Честитки! Точен одговор! За неколку секунди ќе бидете префрлени на НИВО 6!')
                    }
                    forms.splice(i, 1);
                    break;
                }
